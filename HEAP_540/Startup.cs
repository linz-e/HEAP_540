    ...
    public partial class Startup
    {
        ...
        private void createRolesandUsers()
        {
            ...
            if (!roleManager.RoleExists("ITAdmin"))
            {
                ...
                {
                    var result1 = UserManager.AddToRole(user.Id, "ITAdmin");

                }
            }

            // creating Receptionist role    
            if (!roleManager.RoleExists("MedicalReceptionist"))
            {
                var role = new Microsoft.AspNet.Identity.EntityFramework.IdentityRole();
                role.Name = "MedicalReceptionist";
                roleManager.Create(role);
            ...
            // creating Creating Employeee role    
            if (!roleManager.RoleExists("PatientBillingRep"))
            {
                var role = new Microsoft.AspNet.Identity.EntityFramework.IdentityRole();
                role.Name = "PatientBillingRep";
                roleManager.Create(role);
